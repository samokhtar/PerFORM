...
def main():
    ...
    parser.add_argument("--include_val", type=bool, default=False,help="Validation data included during training")
    parser.add_argument("--include_rot", type=int, default=0, help="Include rotations in dataset")
    parser.add_argument("--override_latent", type=int, help="Override number of latents in model")
    ...
    mlp_param, loss_param, rep_param, dataset_param = loadModelParam(model_dir, param_ar = ['mlp_param', 'loss_param', 'rep_param', t_params])
   ...
   # mlp_param['seed'] = args.seed
    ...
    batch = next(iter(data_loader_ml))
    print('Time for next batch: ' + str(time.time()-start_time)+ "\n")
    model_input, sdf, nor, g_name = batch
    print(f"Name: {g_name}")
    ...
    if(os.path.exists(args.model_dir)):
        model = global_cond.loadModel(model_dir = args.model_dir, model_category='LatentNeuralField', model_type = 'train', load_type = args.load_type, epoch = args.epoch, device = device.type, include_val=include_val, include_rot=include_rot, override_latent = args.override_latent)
        model.eval()
    else:
    ...
    max_instances = xyz_sdf_dataset.max_instances  #dataset_param['dataset_ids'] #if(dataset_param['max_num_instances'] != -1) else dataset_param['total_instances']
    print('max_num_instances', max_instances)
    latents = nn.Embedding(num_embeddings = max_instances, embedding_dim = mlp_param['feature_dim'])
    latents.weight.data.normal_(0, mlp_param['latent_init'])
    try:
        m_s = model.latents.weight.shape
        model_test.latents = latents
        model_test = model_test.to(device)
    except:
        model_test.module.latents = latents
